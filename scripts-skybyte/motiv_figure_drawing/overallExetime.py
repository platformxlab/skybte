...
for j, setting in enumerate(settings):
  if inc_base:
    ...
    base = 0
  ax0.bar(x_tick_array[:, j], (data_array[:, j] / data_array[:, 0]), color=color_arr[j], width=bar_width, edgecolor=hatch_color, hatch=hatch_arr[j], label=setting, zorder=3)
# for j, setting in enumerate(settings):
  ax0.bar(x_tick_array[:, j], (data_array[:, j] / data_array[:, 0]), color="none", width=bar_width, edgecolor="white", linewidth=0.8, zorder=3)
...
if inc_base:
  ax0.set_yscale('log')
  ax0.set_yticks([1,2,4,8, 16, 32])
  ax0.set_yticklabels(["1", "2", "4", "8", "16" , "32"])
  ax0.set_ylim([0.5, 80])
else:
  ax0.set_yticks(np.arange(0, 1.8, 0.25))
  ax0.set_ylim([0.1, 2.1])
ax0.set_ylabel("Normalized\nExecution Time", fontsize=23)
...
if inc_base:
  legend = ax0.legend(handles, labels, frameon=False, loc="upper center", bbox_to_anchor=(0.5, 1.08), ncol=3, columnspacing=2.5)
else:
  legend = ax0.legend(handles, labels, frameon=False, loc="upper center", bbox_to_anchor=(0.5, 1.1), ncol=4, columnspacing=1.5)
...
if inc_base:
  fig.savefig(f"output/Exetime.png", bbox_inches=extent)
  fig.savefig(f"output/Exetime.pdf", bbox_inches=extent)
else:
